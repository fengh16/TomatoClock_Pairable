...
<body>
    ...
    <!--
        ...
        function setShowTime(secondsLasted) {
            timeP.innerHTML = getShowTime(secondsLasted);
        }
        // Set the info text
        function infoText(text) {
            infoT.innerHTML = text;
        }
        // Set button text
    ...
    <!--
        function pairTimeGet() {
            if(timePairLeft >= 0) {
                document.getElementById("pairtime").innerHTML = getShowTime(timePairLeft);
            }
            timePairLeft -= 1;
            if(timePairLeft < 0) {
                location.reload();
        ...
        function pairStateGet() {
            if(stateGetCount >= 0) {
                document.getElementById("pairstate").innerHTML = "对方未使用番茄在线，" + stateGetCount + "秒后刷新";
            }
            stateGetCount -= 1;
            if(stateGetCount < 0) {
                location.reload();
        ...
        if(me) {
            ...
            if (document.getElementById("mNowState").value != "NoData" && (Math.floor(parseInt(document.getElementById("mTimeToDo").value)) > Math.floor(parseInt(document.getElementById("mStartTime").innerHTML)))) {
                nowTomato = Math.floor(parseInt(document.getElementById("mNowTomato").value));
                nowSec = Math.floor(parseInt(document.getElementById("mTimeToDo").value)) - Math.floor(parseInt(document.getElementById("mStartTime").innerHTML));
                setShowTime(nowSec);
        ...
        if(other){
            if(document.getElementById("pairstate").innerHTML == "NoData中...") {
                document.getElementById("pairstate").hidden = false;
                var stateGetCount = 600;
                document.getElementById("pairstate").innerHTML = "正在加载对方状态...";
                setInterval(function(){pairStateGet();}, 1000);
            }
            else{
                var timePairLeft = Math.floor(parseInt(document.getElementById("pairtime").innerHTML)) - Math.floor(parseInt(document.getElementById("starttime").innerHTML));
                if(timePairLeft >= 0) {
                    pairTimeGet();
                    ...
                    document.getElementById("pairstate").hidden = false;
                    var stateGetCount = 600;
                    document.getElementById("pairstate").innerHTML = "对方目前没有使用番茄在线，600秒后自动刷新...";
                    setInterval(function(){pairStateGet();}, 1000);
                }
            }
        }
